...
function Signup() {
  ...
  const handleSubmit = async (event) => {
    ...
    if (Object.keys(validationErrors).length === 0) {
      try {
        const response = await axios.post('http://localhost:2001/login', values);
  ...
  return (
    <LoginContainer>
      ...
      <FormContainer>
        ...
        <header>
          <h2 style={{ color: '#333', fontSize: '24px', fontWeight: 'bold', marginBottom: '20px',fontFamily: 'Playfair Display, serif'  }}>Signup To Your Account</h2>
