...
const TeacherProfile = ({ title }) => {
  ...
  const fetchStaff = async () => {
    try {
      const response = await fetch('http://localhost:2001/api/staff');
      if (!response.ok) {
  ...
  const filteredStaff = staff.filter((staff) => {
    return (
      (staff.Staff_ID?.toString().toLowerCase().includes(searchStaffId.toLowerCase()) || false) &&
      (staff.First_Name?.toLowerCase().includes(searchFirstName.toLowerCase()) || false) &&
