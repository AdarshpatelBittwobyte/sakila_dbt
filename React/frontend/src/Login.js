...
const Message = styled.p`
  ...
  color: red;
`;
function Login({ setIsAuthenticated }) { // Receive setIsAuthenticated callback
  ...
  const handleSubmit = async event => {
    ...
    if (Object.keys(validationErrors).length === 0) {
      try {
        ...
        const response = await axios.post(`http://localhost:2001/${loginApi}`, values);
        console.log(response);
        if (response.status === 200) {
          // Authentication successful, call setIsAuthenticated(true)
