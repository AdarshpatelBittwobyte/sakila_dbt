...
mssql_config = config.get("sql_server", {})
...
def create_engine_for_source(source_config):
    if source_config["type"] == "mysql":
        ...
        url = f"postgresql://{source_config['username']}:{source_config['password']}@{source_config['host']}:{source_config['port']}/{source_config['database']}"
    elif source_config["type"] == "sql_server":
        url = f"mssql+pyodbc+pyodbc://{source_config['username']}:{source_config['password']}@{source_config['host']}/{source_config['database']}?driver={source_config['driver']}"
    else:
...
if selected_source_index == 1:
    ...
    selected_config = postgres_config
elif selected_source_index == 3:
    selected_source = "sql_server"
