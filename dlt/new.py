...
def load_database_table(credential, db_name, incremental_col, full_refresh):
    try:
        ...
        for table_name in source.resources.keys():
            ...
            engine1 = create_engine(db_credentials.connection_string2)
...
if __name__ == "__main__":
    ...
    with engine.connect() as conn:
        query = text("select db_name, db_schema, credential, incremental_col, full_refresh from prod.source_cred")
        result = conn.execute(query)
